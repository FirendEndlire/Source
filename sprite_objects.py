...
class Sprites:
    def __init__(self):
        self.sprite_parameters = { # это все характеристи спрайтов, тоже сделаю поддержку своих, если смогу
            ...
            'Starter': {
                'sprite': pygame.image.load(f'data/sprites/DialogNPS/LifeSaver.png').convert_alpha(),
                'viewing_angles': None,
                'shift': 0.6,
                'scale': (0.6, 0.6),
                'side': 30,
                'animation': deque([pygame.image.load(f'data/sprites/DialogNPS/LifeSaver.png').convert_alpha() for i in range(8)]),
                'death_animation': [],
                'is_dead': 'immortal',
                'dead_shift': None,
                'animation_dist': 800,
            ...
            'Barter': {
                'sprite': pygame.image.load(f'data/sprites/DialogNPS/MachineSeller.png').convert_alpha(),
                'viewing_angles': None,
                'shift': 0.6,
                'scale': (0.6, 0.6),
                'side': 30,
                'animation': deque([pygame.image.load(f'data/sprites/DialogNPS/MachineSeller.png').convert_alpha() for i in range(8)]),
                'death_animation': [],
                'is_dead': 'immortal',
                'dead_shift': None,
                'animation_dist': 800,
        ...
        self.list_of_objects = [ #список ВСЕХ обьектов на карте #TODO сделай чтобы по-нормальному хранить это все в папке с картой
            ...
            SpriteObject(self.sprite_parameters['Starter'], (2.5, 2.5)),
            SpriteObject(self.sprite_parameters['Barter'], (4.5, 3.5)),
    ...
    def dialog_teleport(self, sc, player, drawing=None, sprites=None, ray_casting_walls=None):
        ...
        if self.flag == 'dialog_npc':
            if self.count == 0:
                ...
                self.say(elseDialog, sc, player, self.drawing, self.sprites, self.ray_casting_walls, True)
            self.count += 1
